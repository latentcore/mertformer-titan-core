...
| `reports/ocean_pre45k_h200_20260514_partial_evidence.md` | partial Ocean/H200 operational evidence pointer | operators | manual | supporting | Records 2026-05-14 partial startup/training evidence without claiming final proof, benchmark, or checkpoint completion. |
| `reports/ocean_pre45k_h200_20260514_clean_summary.json` | machine-readable partial Ocean/H200 evidence summary | automation | manual | supporting | Mirrors the public Gist summary while excluding the private raw terminal log. |
| `reports/closure_release_truthsync_master_protocol.md` | external closure protocol pointer | operators | manual | supporting | Links the external operator protocol as supporting context; it does not override AGENTS.md or current readiness outputs. |
| `private/commercial/outreach_compute_sponsorship_messages.md` | claim-safe compute and referral outreach snippets | operators | manual | supporting | Keeps LinkedIn/referral/compute outreach evidence-first and prevents raw-log or capability overclaiming. |
...
| `adr/ADR-0004-blocker-fix-pass-core-override.md` | architecture decision record: ADR-0004 Authorized Blocker-Fix Pass: Deliberate Override of the Architecture-Freeze / Protected-Core Constraints | operators | manual | authoritative | Decision record; changes require a superseding ADR or explicit governance exception. |
| `adr/ADR-0005-parallel-precompute-orchestration.md` | architecture decision record: ADR-0005 Multi-GPU Data-Parallel Teacher-Logit Precompute (Additive Orchestration) | operators | manual | authoritative | Decision record; changes require a superseding ADR or explicit governance exception. |
