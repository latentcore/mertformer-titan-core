...
| Stage 5 | `gorilla-llm/gorilla-openfunctions-v2` | train | HF metadata fetch (2026-07-18) | 🔴 **401 Unauthorized on both API and web page — repo not resolvable at this ID today.** Needs Mert's decision: re-identify the correct dataset ID, or drop from the Stage 5 curriculum. | unknown (repo unreachable) |
| Stage 5 | `NousResearch/FC-1k` | train | HF metadata fetch (2026-07-18) | 🔴 **401 Unauthorized on both API and web page — repo not resolvable at this ID today.** Same decision needed as above. | unknown (repo unreachable) |
...
- **2 datasets need Mert's decision, not a code fix:** `gorilla-llm/gorilla-openfunctions-v2` and `NousResearch/FC-1k` both return 401 on the Hugging Face Hub today (checked live 2026-07-18, both API and web UI) — either gated/private now, renamed, or removed. The pipeline cannot pull real data from these IDs as currently configured. Decide: find the correct current ID, or drop these 2 sources from Stage 5.
