...
| `project_actionability` | `true` | `false` | Project blockers now have an execution order, dependency graph, lane board, phase plan, phase readiness scoreboard, owner accountability matrix, and owner work queue wired into the repo-side truth chain. |
