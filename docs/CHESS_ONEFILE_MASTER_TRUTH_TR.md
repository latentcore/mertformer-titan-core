...
| `project_actionability` | `true` | `false` | Proje blocker’ları artık execution order, dependency graph, lane board, phase plan, phase readiness scoreboard, owner accountability matrix ve owner work queue ile repo-side truth zincirine bağlanmıştır. |
