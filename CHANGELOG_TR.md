...
### Eklenenler
- `README.md`/`README_TR.md` Quickstart'ına, zaten var olan `scripts/train_smoke.py` sanity loop'una işaret eden bir satır eklendi (`scripts/README.md`'de zaten belgeliydi, README'den hiç görünmüyordu) — ilk kez bakan biri için "gerçekten eğitiliyor mu" smoke check'i, yeni kod yok. `ARCHITECTURE.md`: mevcut "At a glance" tablosunun altına, onu tamamlayan kompakt bir ASCII katman-akış şeması eklendi. Not: `README_CHECKLIST.md` (superseded, tarihi kayıt, kanonik yüzey değil) ASCII/Mermaid diyagramlarının 178KB→4KB kesiminde `README.md`'nin kendisinden bilinçli olarak kaldırıldığını belgeliyor; bu şema onun yerine boyut kısıtı olmayan `ARCHITECTURE.md`'ye eklendi, yani önceki karar değişmedi. `ARCHITECTURE.md` bilinçli olarak EN-only kalmaya devam ediyor (önceden de öyleydi, `README_TR.md`'nin kendi Canonical surfaces listesindeki `TECHNICAL_REPORT_TR.md`/`MODEL_CARD_TR.md` kardeş örüntüsüyle eşleşiyor — her kanonik dokümanın TR ikizi yok). Claim/kod değişikliği yok.
...
- `scripts/check_overlay_validity.py::check_overlay()`: kullanılmayan, ölü bir `env` sözlüğü silindi (hardcode Unix-only `PATH: "/usr/bin:/bin:/usr/local/bin"`) — tanımlanıyor ama hiç `subprocess.run()`'a geçirilmiyordu (çağrı zaten `full_env` kullanıyor) — `ruff --select F841` ile doğrulandı (reponun varsayılan lint kapsamı, `pyproject.toml`'daki `select = ["E9", "F821", "F822", "F823"]`, bu sınıfı legacy script'lerde düşük-sinyal gürültüden kaçınmak için bilerek dışlıyor; bu tek örnek zaten elle bulunmuş olduğu için doğrudan düzeltildi). `apps/chess_gui/play_mertformer_chess_web.py`: `--device` CLI flag'inin `argparse` `choices`'ında `"cuda"` eksikti (sadece `["cpu", "mps"]`) — otomatik-seçim yolu (`choose_device()`) zaten önce CUDA'yı kontrol ediyor, ama kullanıcı bunu elle zorlayamıyordu. Repo-geneli bir `ruff`/`bandit` keşif taraması (130 kullanılmayan-import/değişken bulgusu, 543 low/medium `bandit` bulgusu) çalıştırıldı ama yukarıdaki tek örnek dışında bilerek işlem yapılmadı — ikisi de projenin kendi belgelenmiş geniş-temizlik-yapma politikasının dışında kalıyor ve bu pass'in `layers/`/`model/`/`train/`/`orchestrator/`/`mertformer_sdk/` dokunmama sınırının dışında. Doğrulandı: `tests/test_chess_gui_contract.py` + `tests/test_config_overlay_strict.py` (10 passed), `scripts/check_overlay_validity.py` doğrudan çalıştırıldı (4/4 overlay OK), tam suite `721 passed, 9 skipped, 1 xfailed` (birebir aynı sayı, regresyon yok), doc-claim consistency OK (yerelde yeniden üretilmiş, hiç commit edilmemiş `reports/train_readiness_decision.{json,md}`'yi kanonik haline geri aldıktan sonra — bu dosyanın kendi bakım geçmişinin daha önce de rastladığı bilinen makine-yerel-durum artifact'ı, gerçek bir doküman bug'ı değil).
