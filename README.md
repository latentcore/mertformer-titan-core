...
- Latest ablation (FINAL, 2026-06-15): 12-seed Liquid/CfC-mixer (`use_liquid`) ON-vs-OFF — held-out ID OFF 96.32% / ON 94.69% (Δ −1.63 pp, p=0.305), OOD 0/0, ON ~30% slower per wall-clock; **no visible accuracy benefit, inconclusive at toy scale, not a benchmark claim** (MoE + the Conv1d LiquidRouter stay on in both arms; this measures the CfC mixer, not the router). **No Liquid speed claim until a verified 45K run.** Canonical: [ABLATION.md](ABLATION.md). The earlier single-seed pilot (Δ+0.50) is superseded.
...
`scripts/chess_5080_onefile.py` now supports named bundle overlays and explicit per-flag overrides for the mirrored advanced surfaces.

- Bundle CLI: `--feature-bundle <name>`
- Per-flag CLI: `--enable-features flag_a,flag_b` and `--disable-features flag_c`
- Canonical 24h RTX 4060 train-start profile: `strength_4060_24h`
- Supported portable baseline profile: `production_5080`
...
- Research-only 24h RTX 4060 profiles: `strength_4060_24h_all_on_experimental` and `strength_4060_24h_omni_max`
...
- Canonical operator runbook/checklist path: `runbooks/chess_4060_24h.md` and `checklists/chess_4060_24h.md`
- Research-only runbook/checklist path: `runbooks/chess_4060_24h_all_on_experimental.md` and `checklists/chess_4060_24h_all_on_experimental.md`
