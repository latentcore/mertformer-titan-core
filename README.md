...
- Stable README config anchor: `use_torch_compile = False`
...
MertFormer Titan is a systems-and-evidence repository with three connected lanes:
- low-bit systems and runtime infrastructure
- offline assistant and RAG foundations
- a contained chess proof lane used to demonstrate evaluation discipline and training-system behavior

The current strong signal is not a headline trained-model benchmark. The current strong signal is that the repository already behaves like a disciplined ML systems project: readiness gates are explicit, verification is script-owned, fallback behavior is documented, and claim boundaries are enforced instead of narrated away.

## Current Exact State
- Repo-wide verification and closure surfaces are wired.
- Current readiness verdict is `TRAIN_ALLOWED`.
- Current readiness reason is `READY_OFFLINE_CLEAN`.
- The canonical path is `offline-clean`; the teacher lane is optional and gated.
- Exact `45K` remains the preferred serious validation target, but application readiness is defined by real owned training evidence tied to checkpoints rather than the step count alone.
- There is no trained main checkpoint claim in this repository yet.
- There is no claim-grade headline benchmark claim for the main model yet.
- Chess evidence exists as a contained proof-of-system lane; it is not narrated as a substitute for a trained main language-model checkpoint.

## Review Lanes
### 1. Systems and Scaling
The repo includes low-bit runtime paths, backend routing, verification gates, telemetry surfaces, and train-readiness contracts intended to make efficiency and failure modes inspectable.

### 2. Offline Assistant Foundations
Tool governance, local retrieval, observability, and assistant-side infrastructure are present as real system surfaces. They are intentionally described as foundations, not as a finished product launch.

### 3. Chess Proof Lane
The chess lane exists to demonstrate contained training behavior, evaluation honesty, and evidence discipline under a narrower workload. It should be read as a system-proof lane, not as a claim that the main model has already completed frontier-scale language-model training.

## Architecture Snapshot
The core architectural ideas currently visible in code and docs are:
- BitNet-style low-bit layers and runtime discipline
- `LiquidRouter` for temporal routing inside sparse MoE flows
- `MLA`-labeled attention with the current implementation explicitly disclosed as GQA-based
- training and release governance built around manifests, reports, hashes, and closure scripts

## Current Measured Evidence
- Latest canonical test stat marker: `203 passed, 3 skipped`
- `python3 -m pytest -q` passed locally in the latest closure pass.
- `.titan-venv/bin/python -m ruff check .` passed in the latest closure pass.
- `bash scripts/verify_all.sh` is the canonical repo verification gate and currently passes in the latest closure pass.
- Repo-side training readiness is green for the offline-clean lane.
- Generated closure artifacts, manifests, and package hashes are rebuilt by script rather than hand-maintained.

See the exact evidence boundary in:
...
## What Is Still Open
The remaining high-value open class is not missing folders or missing scripts. It is the post-run evidence class:
- real owned training run completion
- trained final weights
- best/latest checkpoint proof
- checkpoint-bound benchmark outputs
- trained demo bundle
- trained export/device measurements as a strong plus

Until that evidence exists, the main model stays outside claim-grade benchmark narration.
...
For the shortest public summary, use:
- [README_SUMMARY.md](README_SUMMARY.md)
- [README_SUMMARY_TR.md](README_SUMMARY_TR.md)

For operational entrypoints and truth docs, use:
- [START_HERE.md](START_HERE.md)
- [docs/CHESS_ONEFILE_MASTER_TRUTH.md](docs/CHESS_ONEFILE_MASTER_TRUTH.md)
- [docs/PROJECT_MASTER_TRUTH.md](docs/PROJECT_MASTER_TRUTH.md)
- [reports/final_truth_matrix.md](reports/final_truth_matrix.md)
- [reports/known_limits_v1.md](reports/known_limits_v1.md)
- [reports/final_backlog_missing_items.md](reports/final_backlog_missing_items.md)

For deeper technical and application-facing material, use:
...
## License
This repository is proprietary and confidential. See [LICENSE](LICENSE) and [LICENSE_TR](LICENSE_TR) for the legal boundary.
