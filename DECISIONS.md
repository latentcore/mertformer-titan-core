...
- **Left in place — generated, tightly coupled to the verified closure ladder — DOCUMENT, do NOT
  ...
  > ⚠ **DÜZELTME (2026-07-27):** Bu maddenin "`reports/commercial_handover/` yerinde kalır" kararı
  > **bir gün sonra fiilen tersine döndü** — 2026-06-17 (commit `90bf7c0`, bkz. aşağıdaki "Repo
  > curation — second pass" bölümü) `reports/commercial_handover/` klasörünü (11 dosya)
  > `private/commercial/commercial_handover/`'a taşıdı (gitignore'lu, untracked), bu maddenin
  > açıkça "DO NOT rewire" dediği tam kararı bozarak. Sonuç: `build_closure_governance_pack.py`'nin
  > kendi scorecard kanıt kontrolü (`support_maintenance_policy` kalemi) + `reports/quality_gate_matrix.md`
  > + `reports/commercial_handover_pack.md` en az bir ay boyunca ölü bir path'e işaret etti —
  > `reports/repo_closure_scorecard.md`'nin `all_green: false` (23/24) kalmasının tek sebebiydi,
  > hiçbir BACKLOG/DECISIONS girdisi bunu fark etmedi (2026-07-27 X-ray derin analiz pass'inde
  > bulundu). **Düzeltme yaklaşımı:** klasörü geri taşımak (daha büyük, daha yeni bir single-persona
  > kararını tersine çevirir) yerine, üç kanıt-path referansı (`build_closure_governance_pack.py`,
  > `build_offline_closure_pack.py`'nin `commercial_handover_pack.md` üretici fonksiyonu,
  > `reports/quality_gate_matrix.md`'nin üretici fonksiyonu) dosyanın gerçek şu anki konumuna
  > (`private/commercial/commercial_handover/`) repoint edildi. Bkz. §0.10 — closure belgeleri de
  > yanılabilir, çelişki tarih sırasıyla düzeltilir, tarihçe silinmez.
...
- **Verified real but LEFT (quarantine / freeze / external):** `orchestrator/core.py` gpt2-fallback +
  ...
  > **Extended (2026-07-27):** the "..." above was never enumerated, leaving `AGENTS.md`, `ENV_VARS.md`,
  > `OFFLINE_4060_DEMO.md`, and `V2_BACKLOG_SEED.md` ambiguous (no explicit EN-only-intentional record,
  > no `_TR` twin either) when a root-`*.md` audit checked bilingual-pair coverage. Decision: these four
  > are EN-only-intentional for the same reason as ARCHITECTURE/START_HERE — they are contributor/agent-
  > facing operational references (project-closure rules, env-var index, a local demo runbook, a raw
  > backlog seed), not reviewer-facing narrative surfaces, and none has been requested in Turkish. Not
  > translated now; if a genuine need for a Turkish twin of any of these four surfaces comes up, treat it
  > as a fresh request rather than assuming this note authorizes silently adding one later without review.
...
- **Deliberately DEFERRED (with reason):** ADR-0005 single-naming-mode assert (needs a careful naming-mode detector in the precompute write path — a wrong assert there is riskier than the gap); `p100_safe max_steps` "collision" (a judgment call about a demo profile's correct number, low value); D5 auto-batch-from-config and D7 resume-order (frozen-path — they change what the next run computes, so they wait for the RTX-5070 re-run to verify `bdee57a` first); `report_builder` baseline (already honestly labelled `hardcoded_target_threshold_not_measured` — not a bug).
  > ⚠ **SUPERSEDED (2026-07-25):** see the matching correction in `BACKLOG.md`'s "Pre-45K safe pass — DONE (2026-07-09)" section — 3 of these 4 items (ADR-0005, `p100_safe max_steps`, D5) were fixed for real in commit `8e8978f` (2026-07-11), and D7 was independently audited and confirmed already correct in that same pass. This entry stayed unedited for two weeks after the fix landed; not caught until a live re-check on 2026-07-25.
...
- **Documented, deliberately NOT fixed — new safety-relevant finding.** *(⚠ SUPERSEDED 2026-07-25 — see BACKLOG.md "C1 — divergence guard's grad_norm blind spot": Mert explicitly reviewed and approved a design in-session, and it was applied as a candidate fix. This entry is kept verbatim as the historical record of the finding; it is no longer an open gap as of C1.)* `utils/divergence_guard.py` was read in full (`update_divergence_guard_state()` and its docstring) and confirmed to be purely `loss_ema`-based — no `grad_norm` parameter anywhere. In both runs above, the guard armed on an already-slightly-elevated reference (contaminated by early-onset instability) and then **never tripped**, because `loss_ema` never exceeded `1.5×` that reference even while `GradNorm` was in the trillions — a real blind spot to GradNorm-only explosions when `clip=2.0` keeps the loss signal artificially bounded. Not patched: modifying a safety-critical circuit breaker with no further real-hardware run available to verify the change in this pass was judged too risky; the gap is documented in `BACKLOG.md` instead, as a new open item, not silently closed.
  > ⚠ FURTHER UPDATE (2026-07-25, later the same day): C1 was verified working on real hardware — see BACKLOG.md "Third real-hardware confirmation (2026-07-25, Kaggle T4×2)". Both the 36M and 171M re-verification jobs diverged again (LR/z-loss candidates alone insufficient — a fourth negative data point on this divergence class), but C1's grad-norm co-trigger fired correctly both times (36M at step ~416, 171M at step ~530), each producing a clean checkpoint save instead of an uncontrolled explosion. C1 itself moves from "candidate, applied, unverified" to "candidate, applied, verified working correctly on real hardware, at two different scales." The underlying divergence is still not resolved — only the safety response to it is now confirmed. Full evidence: `evidence/2026-07-25-lm-36m-reverify/`, `evidence/2026-07-25-lm-171m-reverify/`.
...
### Param-count estimator fix + full root-doc cross-consistency pass (2026-07-27)

**Decision: fix the shared-expert/moe_intermediate undercount in both analytical param estimators, then close every real cross-file contradiction a full root-`*.md` audit surfaces — all doc-only, no training-math touched.**

- **The bug (fixed, commit `198e06c`):** `scripts/scaling_audit_math.py::estimate_params()` and `config/config.py::_estimate_total_params()` both (a) sized MoE experts off the dense `intermediate_size` instead of the real, larger `moe_intermediate`, and (b) omitted `layers/moe.py`'s always-active shared expert entirely. Active params were undercounted ~44%, total params ~8%. Fixed in both; `scaling_audit_math.py` now agrees with `ARCHITECTURE.md`'s independently-sourced ~1.86B active-param figure to within <1%. 4 new tests (`622 → 626 passed, 5 skipped`).
- **The governance miss, self-identified:** that fix commit landed with zero `BACKLOG.md`/`DECISIONS.md`/`CHANGELOG.md` entry — immediately after the prior commit (`f8890af`) added the CHANGELOG-drift-prevention note this exact class of miss is about. Closed by this entry + the matching `BACKLOG.md`/`CHANGELOG.md`/`CHANGELOG_TR.md` entries dated the same day.
- **Six further doc-only findings, each verified live before fixing (no assumption carried through unverified):**
  1. `CHANGELOG.md`/`CHANGELOG_TR.md` — "Pass 7 (2026-06-13)" was mis-filed after the 2026-03-13/2026-02-08 tagged entries, breaking the file's own reverse-chronological order. Moved to its correct slot, both languages.
  2. `ENV_VARS.md` — a full month stale (last content update 2026-06-28), missing the entire 2026-07-08→07-25 stabilization-pass env-var surface despite several of the missing vars being named explicitly in `BACKLOG.md`, `TRAINING_PLAN.md`, and `START_HERE.md` itself. Re-synced against a live `grep` of every `os.environ.get`/`os.getenv` call in the repo (not a partial/assumed list).
  3. `V2_BACKLOG_SEED.md` Track F — 3 items (`liquid_warmup_steps` env override, z-loss double-multiply, `mark_weights_updated()` question) were still shown open despite being resolved/investigated per this file's own 2026-07-12 entries. Annotated resolved with cross-references.
  4. `CHESS_5080_POC_INTERNAL.md` (EN) — its own TR twin already carried an explicit inline note flagging the EN file as stale on Share Export behavior. Brought to parity rather than leaving a self-admitted-but-unfixed gap sitting in the docs.
  5. `TECHNICAL_REPORT.md`/`_TR` — masthead date frozen at 2026-06-18 while the body already carried a 2026-07-19 revision. Added an explicit last-revision note (same discipline as the append-only correction pattern used for git-history preservation elsewhere in this repo's process, and for the Documents-external Master Protocol addenda).
