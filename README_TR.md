...
- Son ablasyon (NİHAİ, 2026-06-15): 12-seed Liquid/CfC-mixer (`use_liquid`) AÇIK-vs-KAPALI — held-out ID KAPALI %96.32 / AÇIK %94.69 (Δ −1.63 pp, p=0.305), OOD %0/%0, AÇIK duvar-saatinde ~%30 yavaş; **gözle görülür doğruluk faydası yok, toy ölçekte sonuçsuz, benchmark iddiası değil** (MoE + Conv1d LiquidRouter iki kolda da açık; bu, router'ı değil CfC mixer'ı ölçer). **Doğrulanmış 45K koşusuna kadar Liquid hız iddiası yok.** Kanonik: [ABLATION_TR.md](ABLATION_TR.md). Önceki tek-seed pilot (Δ+0.50) ezildi.
...
│   ├── trainer_core.py  # Python modülü/scripti (trainer core için modül)
│   ├── trainer_data.py  # Python modülü/scripti (trainer data için modül)
│   └── trainer_eval.py  # Python modülü/scripti (trainer eval için modül)
