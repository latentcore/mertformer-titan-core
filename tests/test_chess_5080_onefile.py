...
def test_write_closure_manifests_marks_closure_artifacts_present(tmp_path: Path) -> None:
    ...
    assert entries['project_closure_phase_plan']['exists'] is False
    ...
    assert entries['project_ready_now_board']['exists'] is False
    assert entries['project_unlock_impact_report']['exists'] is False
    assert entries['project_parallel_workset_report']['exists'] is False
    ...
    assert entries['project_evidence_criticality_report']['exists'] is False
    assert entries['project_phase_transition_matrix']['exists'] is False
    assert entries['project_owner_load_report']['exists'] is False
    assert entries['project_phase_dependency_pressure_report']['exists'] is False
    assert entries['project_owner_bottleneck_alignment_report']['exists'] is False
    assert entries['project_evidence_phase_heatmap_report']['exists'] is False
    assert entries['project_blocker_risk_register_report']['exists'] is False
    assert entries['project_release_prereq_matrix_report']['exists'] is False
    assert entries['project_foundation_run_dependency_report']['exists'] is False
    assert entries['project_release_path_report']['exists'] is False
    assert entries['project_external_closure_cluster_report']['exists'] is False
    assert entries['project_owner_evidence_gap_report']['exists'] is False
    assert entries['project_release_gate_dependency_report']['exists'] is False
    assert entries['project_external_signoff_queue_report']['exists'] is False
    assert entries['project_release_evidence_bridge_report']['exists'] is False
    assert entries['project_training_run_readiness_report']['exists'] is False
    assert entries['project_benchmark_closure_dependency_report']['exists'] is False
    assert entries['project_release_decision_queue_report']['exists'] is False
...
def test_write_release_evidence_reports_writes_release_surfaces(tmp_path: Path) -> None:
    ...
    project_closure_phase_plan = json.loads((layout.reports_dir / 'project_closure_phase_plan.json').read_text(encoding='utf-8'))
    ...
    project_ready_now_board = json.loads((layout.reports_dir / 'project_ready_now_board.json').read_text(encoding='utf-8'))
    project_unlock_impact_report = json.loads((layout.reports_dir / 'project_unlock_impact_report.json').read_text(encoding='utf-8'))
    project_parallel_workset_report = json.loads((layout.reports_dir / 'project_parallel_workset_report.json').read_text(encoding='utf-8'))
    ...
    project_evidence_criticality_report = json.loads((layout.reports_dir / 'project_evidence_criticality_report.json').read_text(encoding='utf-8'))
    project_phase_transition_matrix = json.loads((layout.reports_dir / 'project_phase_transition_matrix.json').read_text(encoding='utf-8'))
    project_owner_load_report = json.loads((layout.reports_dir / 'project_owner_load_report.json').read_text(encoding='utf-8'))
    project_phase_dependency_pressure_report = json.loads((layout.reports_dir / 'project_phase_dependency_pressure_report.json').read_text(encoding='utf-8'))
    project_owner_bottleneck_alignment_report = json.loads((layout.reports_dir / 'project_owner_bottleneck_alignment_report.json').read_text(encoding='utf-8'))
    project_evidence_phase_heatmap_report = json.loads((layout.reports_dir / 'project_evidence_phase_heatmap_report.json').read_text(encoding='utf-8'))
    project_blocker_risk_register_report = json.loads((layout.reports_dir / 'project_blocker_risk_register_report.json').read_text(encoding='utf-8'))
    project_release_prereq_matrix_report = json.loads((layout.reports_dir / 'project_release_prereq_matrix_report.json').read_text(encoding='utf-8'))
    project_foundation_run_dependency_report = json.loads((layout.reports_dir / 'project_foundation_run_dependency_report.json').read_text(encoding='utf-8'))
    project_release_path_report = json.loads((layout.reports_dir / 'project_release_path_report.json').read_text(encoding='utf-8'))
    project_external_closure_cluster_report = json.loads((layout.reports_dir / 'project_external_closure_cluster_report.json').read_text(encoding='utf-8'))
    project_owner_evidence_gap_report = json.loads((layout.reports_dir / 'project_owner_evidence_gap_report.json').read_text(encoding='utf-8'))
    project_release_gate_dependency_report = json.loads((layout.reports_dir / 'project_release_gate_dependency_report.json').read_text(encoding='utf-8'))
    project_external_signoff_queue_report = json.loads((layout.reports_dir / 'project_external_signoff_queue_report.json').read_text(encoding='utf-8'))
    project_release_evidence_bridge_report = json.loads((layout.reports_dir / 'project_release_evidence_bridge_report.json').read_text(encoding='utf-8'))
    project_training_run_readiness_report = json.loads((layout.reports_dir / 'project_training_run_readiness_report.json').read_text(encoding='utf-8'))
    project_benchmark_closure_dependency_report = json.loads((layout.reports_dir / 'project_benchmark_closure_dependency_report.json').read_text(encoding='utf-8'))
    project_release_decision_queue_report = json.loads((layout.reports_dir / 'project_release_decision_queue_report.json').read_text(encoding='utf-8'))
    ...
    assert project_closure_phase_plan['schema'] == 'chess_project_closure_phase_plan_v1'
    ...
    assert project_ready_now_board['schema'] == 'chess_project_ready_now_board_v1'
    assert project_unlock_impact_report['schema'] == 'chess_project_unlock_impact_report_v1'
    assert project_parallel_workset_report['schema'] == 'chess_project_parallel_workset_report_v1'
    ...
    assert project_evidence_criticality_report['schema'] == 'chess_project_evidence_criticality_report_v1'
    assert project_phase_transition_matrix['schema'] == 'chess_project_phase_transition_matrix_v1'
    assert project_owner_load_report['schema'] == 'chess_project_owner_load_report_v1'
    assert project_phase_dependency_pressure_report['schema'] == 'chess_project_phase_dependency_pressure_report_v1'
    assert project_owner_bottleneck_alignment_report['schema'] == 'chess_project_owner_bottleneck_alignment_report_v1'
    assert project_evidence_phase_heatmap_report['schema'] == 'chess_project_evidence_phase_heatmap_report_v1'
    assert project_blocker_risk_register_report['schema'] == 'chess_project_blocker_risk_register_report_v1'
    assert project_release_prereq_matrix_report['schema'] == 'chess_project_release_prereq_matrix_report_v1'
    assert project_foundation_run_dependency_report['schema'] == 'chess_project_foundation_run_dependency_report_v1'
    assert project_release_path_report['schema'] == 'chess_project_release_path_report_v1'
    assert project_external_closure_cluster_report['schema'] == 'chess_project_external_closure_cluster_report_v1'
    assert project_owner_evidence_gap_report['schema'] == 'chess_project_owner_evidence_gap_report_v1'
    assert project_release_gate_dependency_report['schema'] == 'chess_project_release_gate_dependency_report_v1'
    assert project_external_signoff_queue_report['schema'] == 'chess_project_external_signoff_queue_report_v1'
    assert project_release_evidence_bridge_report['schema'] == 'chess_project_release_evidence_bridge_report_v1'
    assert project_training_run_readiness_report['schema'] == 'chess_project_training_run_readiness_report_v1'
    assert project_benchmark_closure_dependency_report['schema'] == 'chess_project_benchmark_closure_dependency_report_v1'
    assert project_release_decision_queue_report['schema'] == 'chess_project_release_decision_queue_report_v1'
    ...
    assert entries['project_closure_phase_plan']['exists'] is True
    ...
    assert entries['project_ready_now_board']['exists'] is True
    assert entries['project_unlock_impact_report']['exists'] is True
    assert entries['project_parallel_workset_report']['exists'] is True
    ...
    assert entries['project_evidence_criticality_report']['exists'] is True
    assert entries['project_phase_transition_matrix']['exists'] is True
    assert entries['project_owner_load_report']['exists'] is True
    assert entries['project_phase_dependency_pressure_report']['exists'] is True
    assert entries['project_owner_bottleneck_alignment_report']['exists'] is True
    assert entries['project_evidence_phase_heatmap_report']['exists'] is True
    assert entries['project_blocker_risk_register_report']['exists'] is True
    assert entries['project_release_prereq_matrix_report']['exists'] is True
    assert entries['project_foundation_run_dependency_report']['exists'] is True
    assert entries['project_release_path_report']['exists'] is True
    assert entries['project_external_closure_cluster_report']['exists'] is True
    assert entries['project_owner_evidence_gap_report']['exists'] is True
    assert entries['project_release_gate_dependency_report']['exists'] is True
    assert entries['project_external_signoff_queue_report']['exists'] is True
    assert entries['project_release_evidence_bridge_report']['exists'] is True
    assert entries['project_training_run_readiness_report']['exists'] is True
    assert entries['project_benchmark_closure_dependency_report']['exists'] is True
    assert entries['project_release_decision_queue_report']['exists'] is True
    ...
    assert closure_gap_summary['project_phase_status'] == 'ready'
    ...
    assert closure_gap_summary['project_ready_now_status'] == 'ready'
    assert closure_gap_summary['project_unlock_impact_status'] == 'ready'
    assert closure_gap_summary['project_parallel_workset_status'] == 'ready'
    assert closure_gap_summary['project_phase_exit_criteria_status'] == 'ready'
    assert closure_gap_summary['project_execution_wave_status'] == 'ready'
    assert closure_gap_summary['project_evidence_backlog_status'] == 'ready'
    assert closure_gap_summary['project_owner_bottleneck_alignment_status'] == 'ready'
    assert closure_gap_summary['project_evidence_phase_heatmap_status'] == 'ready'
    assert closure_gap_summary['project_blocker_risk_register_status'] == 'ready'
    assert closure_gap_summary['project_release_prereq_matrix_status'] == 'ready'
    assert closure_gap_summary['project_foundation_run_dependency_status'] == 'ready'
    assert closure_gap_summary['project_release_path_status'] == 'ready'
    assert closure_gap_summary['project_external_closure_cluster_status'] == 'ready'
    assert closure_gap_summary['project_owner_evidence_gap_status'] == 'ready'
    assert closure_gap_summary['project_release_gate_dependency_status'] == 'ready'
    assert closure_gap_summary['project_external_signoff_queue_status'] == 'ready'
    assert closure_gap_summary['project_release_evidence_bridge_status'] == 'ready'
    assert closure_gap_summary['project_training_run_readiness_status'] == 'ready'
    assert closure_gap_summary['project_benchmark_closure_dependency_status'] == 'ready'
    assert closure_gap_summary['project_release_decision_queue_status'] == 'ready'
    ...
    assert project_closure_phase_plan['phase_count'] >= 5
    assert project_closure_phase_plan['status'] == 'ready'
    assert project_closure_phase_plan['rows'][0]['phase'] == 'foundation_runs'
    assert any(row['phase'] == 'external_validation' and row['blocker_count'] >= 3 for row in project_closure_phase_plan['rows'])
    assert project_phase_readiness_scoreboard['phase_count'] == project_closure_phase_plan['phase_count']
    assert project_phase_readiness_scoreboard['status'] == 'ready'
    assert project_phase_readiness_scoreboard['rows'][0]['phase'] == 'foundation_runs'
    assert any(row['status'] in {'partially_ready', 'dependency_blocked'} for row in project_phase_readiness_scoreboard['rows'])
    assert project_owner_accountability_matrix['owner_count'] >= 6
    assert project_owner_accountability_matrix['covered_blocker_count'] == project_remaining_real_blockers['item_count']
    assert project_owner_accountability_matrix['status'] == 'ready'
    assert any(row['owner_domain'] == 'release_governance' and row['blocker_count'] >= 1 for row in project_owner_accountability_matrix['rows'])
    ...
    assert project_ready_now_board['item_count'] >= 1
    assert project_ready_now_board['status'] == 'ready'
    assert project_ready_now_board['rows'][0]['label'] == 'real_training_outputs_pending'
    assert any(row['label'] == 'real_training_outputs_pending' and row['direct_unlock_count'] >= 1 for row in project_ready_now_board['rows'])
    assert project_unlock_impact_report['item_count'] == project_remaining_real_blockers['item_count']
    assert project_unlock_impact_report['status'] == 'ready'
    assert project_unlock_impact_report['top_unlock_label'] == 'real_training_outputs_pending'
    assert project_unlock_impact_report['rows'][0]['total_unlock_count'] >= project_unlock_impact_report['rows'][0]['direct_unlock_count']
    assert project_parallel_workset_report['workset_count'] >= 1
    assert project_parallel_workset_report['covered_ready_now_count'] == project_ready_now_board['item_count']
    assert project_parallel_workset_report['status'] == 'ready'
    assert any('real_training_outputs_pending' in row['blockers'] for row in project_parallel_workset_report['rows'])
    assert project_phase_exit_criteria_report['phase_count'] == project_closure_phase_plan['phase_count']
    assert project_phase_exit_criteria_report['status'] == 'ready'
    assert project_phase_exit_criteria_report['rows'][0]['phase'] == project_closure_phase_plan['rows'][0]['phase']
    assert all(row['blocker_count'] >= 1 for row in project_phase_exit_criteria_report['rows'])
    assert project_execution_wave_report['wave_count'] >= 2
    assert project_execution_wave_report['covered_blocker_count'] == project_remaining_real_blockers['item_count']
    assert project_execution_wave_report['status'] == 'ready'
    assert project_execution_wave_report['rows'][0]['wave'] == 0
    assert 'real_training_outputs_pending' in project_execution_wave_report['rows'][0]['blockers']
    assert project_evidence_backlog_report['evidence_count'] >= 1
    ...
    assert project_evidence_criticality_report['item_count'] == project_evidence_backlog_report['evidence_count']
    assert project_evidence_criticality_report['status'] == 'ready'
    assert project_evidence_criticality_report['top_evidence_label'] == 'benchmark raw outputs'
    assert project_phase_transition_matrix['phase_count'] == project_closure_phase_plan['phase_count']
    assert project_phase_transition_matrix['status'] == 'ready'
    assert any(row['phase'] == 'foundation_runs' and row['outgoing_phase_count'] >= 1 for row in project_phase_transition_matrix['rows'])
    assert project_owner_load_report['owner_count'] == project_owner_accountability_matrix['owner_count']
    assert project_owner_load_report['status'] == 'ready'
    assert any(row['owner_domain'] == 'release_governance' and row['blocker_count'] >= 1 for row in project_owner_load_report['rows'])
    assert project_phase_dependency_pressure_report['phase_count'] == project_phase_transition_matrix['phase_count']
    assert project_phase_dependency_pressure_report['status'] == 'ready'
    assert any(row['phase'] == 'external_validation' and row['pressure_score'] >= 1 for row in project_phase_dependency_pressure_report['rows'])
    assert project_owner_bottleneck_alignment_report['schema'] == 'chess_project_owner_bottleneck_alignment_report_v1'
    assert project_owner_bottleneck_alignment_report['owner_count'] == project_owner_load_report['owner_count']
    assert project_owner_bottleneck_alignment_report['status'] == 'ready'
    assert any(row['owner_domain'] == 'release_governance' and row['top_owner_bottleneck_label'] for row in project_owner_bottleneck_alignment_report['rows'])
    assert project_evidence_phase_heatmap_report['schema'] == 'chess_project_evidence_phase_heatmap_report_v1'
    assert project_evidence_phase_heatmap_report['phase_count'] == project_phase_transition_matrix['phase_count']
    assert project_evidence_phase_heatmap_report['status'] == 'ready'
    assert any(row['phase'] == 'external_validation' and row['evidence_count'] >= 1 for row in project_evidence_phase_heatmap_report['rows'])
    assert project_blocker_risk_register_report['schema'] == 'chess_project_blocker_risk_register_report_v1'
    assert project_blocker_risk_register_report['item_count'] == project_remaining_real_blockers['item_count']
    assert project_blocker_risk_register_report['status'] == 'ready'
    assert project_blocker_risk_register_report['top_risk_label'] in {item['label'] for item in project_remaining_real_blockers['items']}
    assert any(row['label'] == 'real_training_outputs_pending' and row['risk_tier'] in {'high', 'medium'} for row in project_blocker_risk_register_report['rows'])
    assert project_release_prereq_matrix_report['schema'] == 'chess_project_release_prereq_matrix_report_v1'
    assert project_release_prereq_matrix_report['item_count'] == project_remaining_real_blockers['item_count']
    assert project_release_prereq_matrix_report['direct_release_prereq_count'] >= 1
    assert project_release_prereq_matrix_report['status'] == 'ready'
    assert any(row['label'] == 'rc_golden_final_release_pending' and row['release_distance'] == 0 for row in project_release_prereq_matrix_report['rows'])
    assert project_foundation_run_dependency_report['schema'] == 'chess_project_foundation_run_dependency_report_v1'
    assert project_foundation_run_dependency_report['root_label'] == 'real_training_outputs_pending'
    assert project_foundation_run_dependency_report['dependent_blocker_count'] >= 3
    assert project_foundation_run_dependency_report['status'] == 'ready'
    assert any(row['label'] == 'benchmark_evidence_pending' and row['dependency_distance'] >= 1 for row in project_foundation_run_dependency_report['rows'])
    assert project_release_path_report['schema'] == 'chess_project_release_path_report_v1'
    assert project_release_path_report['path_count'] >= 1
    assert project_release_path_report['terminal_label'] == 'rc_golden_final_release_pending'
    assert project_release_path_report['status'] == 'ready'
    assert any(row['root_label'] == 'real_training_outputs_pending' for row in project_release_path_report['rows'])
    assert project_external_closure_cluster_report['schema'] == 'chess_project_external_closure_cluster_report_v1'
    assert project_external_closure_cluster_report['cluster_count'] == 3
    assert project_external_closure_cluster_report['status'] == 'ready'
    assert any(row['phase'] == 'external_validation' and row['blocker_count'] >= 1 for row in project_external_closure_cluster_report['rows'])
    assert project_owner_evidence_gap_report['schema'] == 'chess_project_owner_evidence_gap_report_v1'
    assert project_owner_evidence_gap_report['owner_count'] == project_owner_accountability_matrix['owner_count']
    assert project_owner_evidence_gap_report['status'] == 'ready'
    assert any(row['owner_domain'] == 'release_governance' and row['evidence_count'] >= 1 for row in project_owner_evidence_gap_report['rows'])
    assert project_release_gate_dependency_report['schema'] == 'chess_project_release_gate_dependency_report_v1'
    assert project_release_gate_dependency_report['item_count'] >= 1
    assert project_release_gate_dependency_report['direct_release_prereq_count'] >= 1
    assert project_release_gate_dependency_report['status'] == 'ready'
    assert any(row['label'] == 'rc_golden_final_release_pending' and row['release_distance'] == 0 for row in project_release_gate_dependency_report['rows'])
    assert project_external_signoff_queue_report['schema'] == 'chess_project_external_signoff_queue_report_v1'
    assert project_external_signoff_queue_report['queue_count'] >= 1
    assert project_external_signoff_queue_report['signoff_class_count'] >= 1
    assert project_external_signoff_queue_report['status'] == 'ready'
    assert any(row['signoff_class'] == 'release_governance' for row in project_external_signoff_queue_report['rows'])
    assert project_release_evidence_bridge_report['schema'] == 'chess_project_release_evidence_bridge_report_v1'
    assert project_release_evidence_bridge_report['item_count'] >= 1
    assert project_release_evidence_bridge_report['top_release_evidence_label'] != ''
    assert project_release_evidence_bridge_report['status'] == 'ready'
    assert any(row['evidence_label'] == project_release_evidence_bridge_report['top_release_evidence_label'] for row in project_release_evidence_bridge_report['rows'])
    assert project_training_run_readiness_report['schema'] == 'chess_project_training_run_readiness_report_v1'
    assert project_training_run_readiness_report['root_label'] == 'real_training_outputs_pending'
    assert project_training_run_readiness_report['item_count'] == project_foundation_run_dependency_report['dependent_blocker_count'] + 1
    assert project_training_run_readiness_report['ready_now_count'] >= 1
    assert project_training_run_readiness_report['status'] == 'ready'
    assert any(row['label'] == 'benchmark_evidence_pending' and row['dependency_distance'] >= 1 for row in project_training_run_readiness_report['rows'])
    assert project_benchmark_closure_dependency_report['schema'] == 'chess_project_benchmark_closure_dependency_report_v1'
    assert project_benchmark_closure_dependency_report['root_label'] == 'benchmark_evidence_pending'
    assert project_benchmark_closure_dependency_report['item_count'] >= 1
    assert project_benchmark_closure_dependency_report['release_reaching_count'] >= 1
    assert project_benchmark_closure_dependency_report['status'] == 'ready'
    assert any(row['label'] == 'rc_golden_final_release_pending' and row['benchmark_dependency_distance'] >= 1 for row in project_benchmark_closure_dependency_report['rows'])
    assert project_release_decision_queue_report['schema'] == 'chess_project_release_decision_queue_report_v1'
    assert project_release_decision_queue_report['item_count'] >= 1
    assert project_release_decision_queue_report['decision_class_count'] >= 1
    assert project_release_decision_queue_report['status'] == 'ready'
    assert any(row['decision_class'] == 'release_governance' for row in project_release_decision_queue_report['rows'])
    assert project_external_validation_readiness_report['schema'] == 'chess_project_external_validation_readiness_report_v1'
    assert project_external_validation_readiness_report['phase'] == 'external_validation'
    assert project_external_validation_readiness_report['item_count'] >= 1
    assert project_external_validation_readiness_report['signoff_queue_count'] >= 1
    assert project_external_validation_readiness_report['status'] == 'ready'
    assert any(row['label'] == 'external_strength_unproven' and row['in_signoff_queue'] is True for row in project_external_validation_readiness_report['rows'])
    assert project_artifact_lock_readiness_report['schema'] == 'chess_project_artifact_lock_readiness_report_v1'
    assert project_artifact_lock_readiness_report['item_count'] == 5
    assert project_artifact_lock_readiness_report['artifact_class_count'] >= 4
    assert project_artifact_lock_readiness_report['direct_release_prereq_count'] >= 3
    assert project_artifact_lock_readiness_report['status'] == 'ready'
    assert any(row['label'] == 'trained_artifact_truth_pending' and row['artifact_lock_class'] == 'artifact_registry' for row in project_artifact_lock_readiness_report['rows'])
    assert project_final_release_cutover_report['schema'] == 'chess_project_final_release_cutover_report_v1'
    assert project_final_release_cutover_report['item_count'] >= project_release_gate_dependency_report['item_count']
    assert project_final_release_cutover_report['cutover_stage_count'] >= 4
    ...
    assert crosscheck_labels['ready_now_board_matches_roots'] is True
    assert crosscheck_labels['unlock_impact_matches_graph'] is True
    assert crosscheck_labels['parallel_workset_covers_ready_now'] is True
    ...
    assert crosscheck_labels['evidence_criticality_matches_backlog'] is True
    assert crosscheck_labels['phase_transition_matrix_matches_phase_plan'] is True
    assert crosscheck_labels['owner_load_matches_owner_matrix'] is True
    assert crosscheck_labels['phase_dependency_pressure_matches_transition_matrix'] is True
    assert crosscheck_labels['owner_bottleneck_alignment_matches_owner_load'] is True
    assert crosscheck_labels['evidence_phase_heatmap_matches_phase_transition'] is True
    assert crosscheck_labels['blocker_risk_register_matches_blockers'] is True
    assert crosscheck_labels['release_prereq_matrix_matches_release_paths'] is True
    assert crosscheck_labels['foundation_run_dependency_matches_unlock_root'] is True
    assert crosscheck_labels['release_path_report_matches_release_target'] is True
    assert crosscheck_labels['external_closure_cluster_matches_phase_plan'] is True
    assert crosscheck_labels['owner_evidence_gap_matches_owner_matrix'] is True
    assert crosscheck_labels['release_gate_dependency_matches_release_prereqs'] is True
    assert crosscheck_labels['external_signoff_queue_matches_external_cluster'] is True
    assert crosscheck_labels['release_evidence_bridge_matches_backlog'] is True
    assert crosscheck_labels['training_run_readiness_matches_foundation_dependencies'] is True
    assert crosscheck_labels['benchmark_closure_dependency_matches_root'] is True
    assert crosscheck_labels['release_decision_queue_matches_release_phases'] is True
