...
**2026-07-31, aynı gün tekrar bakıldı:** bu reponun kendi RTX 4060'ında kanonik `hidden_size`'da yapılan bir ölçüm, train-mode `LiquidMixer`'ı yukarıdaki her iki sayıdan da çok daha yavaş buldu (`GQA`'dan ~797-1620x), artı `seq_len >= 2048`'de bir `OutOfMemoryError`, ve decode-mode ölçümü `LiquidMixer`'ı `GQA`'dan 8-23x daha *hızlı* buldu. Tam sayılar: `BACKLOG_TR.md`. Hâlâ bu notun istediği 45K-ölçek cevabı değil, ama artık sadece "bir bakışı hak ediyor" değil — bakıldı.
