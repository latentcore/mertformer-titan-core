...
# -*- coding: utf-8 -*-
"""
Single-file, zero-argument 171M STABILIZATION PILOT run.

[2026-07-12] Derivative of scripts/preflight_run.py, built specifically to launch
config/model/mertformer_pilot_stabilization.yaml (the repo's own documented
"Go/No-Go" LR-verification pilot, BACKLOG.md / DECISIONS.md I.2.2) with the same
proven glue (dependency bootstrap, dataset staging, offline lock, artifact
collection, REPORT.md, output ZIP, resume) as the 36M infra-preflight, WITHOUT
re-implementing or hand-copying the pilot's architecture: this script sets
MERTFORMER_MODEL_CONFIG and lets the repository's own config overlay loader own
every architectural field (hidden_size, layer count, MoE/Liquid placement, step
budget). phase_config() below asserts the overlay actually applied instead of
silently trusting it, and does NOT re-set any field the overlay already owns —
if config/model/mertformer_pilot_stabilization.yaml changes later, this script's
run config changes with it instead of silently overriding it back (the exact
"looks dynamic, is actually a dead constant" class of bug fixed repo-wide in
commit 8e8978f).

PURPOSE (read this first)
-------------------------
This script answers ONE question, per the pilot yaml's own framing: does the
candidate LR regime (config/config.py: learning_rate, warmup_ratio,
router_lr_multiplier) keep grad_norm finite and the loss descending on the full
architecture (BitNet ternary + sparse 8-expert/top-2 MoE + Liquid/CfC + GQA) at
~171.6M measured parameters — the smallest scale considered architecturally
representative — before spending $150-570 and multiple days on the real 45K run.
It is explicitly NOT a capability/quality run and NOT a scaling proxy for 45K
(see the pilot yaml's own header). The PASS/FAIL verdict below is a mechanical
Go/No-Go read of the yaml's own documented gate: clip-hit rate < 5%, grad_norm
never reaches inf, no persistent Liquid freeze, no MoE collapse, held-out loss
descending, and a clean kill+resume drill — NOT a capability statement.

WHAT IT REUSES FROM THE REPOSITORY (unchanged, source of truth)
---------------------------------------------------------------
* config.config.cfg          - the single config object. Its architecture is
                               owned entirely by config/model/
                               mertformer_pilot_stabilization.yaml via the
                               repository's own MERTFORMER_MODEL_CONFIG overlay
                               mechanism — no architecture code is re-implemented
                               or hand-copied here.
...
#
# Architecture (HIDDEN_SIZE..LIQUID_WARMUP_STEPS, TOTAL_STEPS) is NOT set on
# cfg by this script -- it mirrors config/model/mertformer_pilot_stabilization.yaml
# for REPORTING TEXT ONLY (log lines, REPORT.md). The actual run config comes
# from the yaml via PILOT_MODEL_CONFIG_OVERLAY below; phase_config() asserts
# these mirror values actually match what the overlay produced and fails loudly
# if they don't, instead of silently trusting a copy that could drift from the
# yaml. If the yaml changes, update these to match (verified) or the assertion
# will catch the mismatch.
# =====================================================================
MODEL_NAME = "pilot171m_stabilization"
PILOT_MODEL_CONFIG_OVERLAY = "mertformer_pilot_stabilization.yaml"  # config/model/<this>

HIDDEN_SIZE = 512
NUM_LAYERS = 9
NUM_HEADS = 8
NUM_KV_HEADS = 2           # GQA 8 query / 2 kv heads (canonical is 16q/8kv)
HEAD_DIM = 64              # 512 == 8 * 64
FFN_INTERMEDIATE = 1408    # dense FFN width (~2.75x hidden, mirrors canonical 5632/2048)
MOE_EXPERTS = 8
MOE_ACTIVE = 2             # top-k experts per token
MOE_EVERY_N_LAYERS = 3     # expert layers at 0-indexed block ids 2, 5, 8
MOE_INTERMEDIATE = 2048    # per-expert FFN width (4x hidden, mirrors canonical 8192/2048)
LIQUID_LAYER_IDS = [1, 4, 7]  # recurrent mixer layers (offset from MoE layers, no overlap)
LIQUID_WARMUP_STEPS = 667  # canonical fraction 10000/45000 scaled to this run's 3000 steps

SEQ_LEN = 512
MICRO_BATCH = 16
GRAD_ACCUM = 2
GLOBAL_BATCH = MICRO_BATCH * GRAD_ACCUM  # 32 sequences / optimizer step
TOTAL_STEPS = 3000         # pilot yaml's own max_steps
SAVE_INTERVAL = 500
VAL_INTERVAL = 500
LOG_INTERVAL = 10
...
PLANNED_TOKENS = TOTAL_STEPS * TOKENS_PER_STEP      # 49,152,000 (~49M, one pass)
...
BOOTSTRAP_LIBS = (
    ...
    # [2026-07-12] This pilot's whole point is optimizer-faithful LR verification
    # (train/train.py build_optimizer() uses GaLoreAdamW8bit when cfg.use_galore/
    # use_8bit_adam are both True, which the pilot yaml sets). galore_torch itself is
    # pure-Python (no CUDA-version binding like torch/bitsandbytes), so it is safe to
    # auto-install here -- unlike the 36M preflight, which never turns GaLore on and so
    # never needed it. NOTE: GaLoreAdamW8bit's 8-bit state still needs bitsandbytes
    # underneath (FORBIDDEN_INSTALLS below, matches your CUDA driver, not auto-installed
    # by this script). Without a matching bitsandbytes already on this machine,
    # build_optimizer() degrades gracefully to plain torch.optim.AdamW and PRINTS which
    # class actually ran ("OPTIMIZER ACTIVE: ...") -- check REPORT.md's Optimizer row
    # before trusting an LR verdict as GaLore-faithful.
    ("galore_torch", "galore-torch"),
)
...
# =====================================================================
WORK_DIR = SCRIPT_DIR / "pilot171m_work"
...
OUTPUT_ZIP = SCRIPT_DIR / "pilot171m_run_output.zip"
...
def stage_environment() -> dict[str, str]:
    ...
    env = {
        ...
        # [2026-07-12] Owns the pilot's ENTIRE architecture (hidden_size, layer
        # count, MoE/Liquid placement, max_steps, GaLore/8-bit-Adam, ...) via the
        # repository's own config overlay loader -- read at first `import
        # config.config` in phase_config() below. Nothing architectural is
        # re-set on cfg by this script; see phase_config()'s assertion.
        "MERTFORMER_MODEL_CONFIG": PILOT_MODEL_CONFIG_OVERLAY,
        ...
        # [2026-07-12] The pilot yaml itself sets liquid_train_impl: baseline (applied via
        # MERTFORMER_MODEL_CONFIG's overlay, which wins over this env-var-derived default) --
        # matched here to "baseline" too, so this dict and the config snapshot in
        # run_config.json agree instead of one silently losing to the other.
        "TITAN_LIQUID_TRAIN_IMPL": "baseline",
...
def phase_config(device_info: dict, staged_env: dict) -> None:
    ...
    Architecture (hidden_size, layer count, MoE/Liquid placement, max_steps,
    GaLore/8-bit-Adam) is owned ENTIRELY by config/model/
    mertformer_pilot_stabilization.yaml via MERTFORMER_MODEL_CONFIG (set in
    stage_environment(), applied by config.config's own overlay loader at the
    `from config.config import cfg` below) -- not re-set here. Only run-harness
    concerns this script controls independently of the pilot's architecture
    (batching/schedule the yaml doesn't specify, teacher policy, output paths)
    are set below. This intentionally mirrors the repository's own established
    post-import-mutation pattern for reduced-size runs, but for architecture
    specifically defers to the yaml instead of duplicating it, so a future
    edit to the yaml changes the run instead of being silently overridden back
    (see module docstring).
    """
    from config.config import cfg, validate_layer_config  # noqa: WPS433
    from utils.logger import try_git_commit  # noqa: WPS433

    # Real verification, not a trust-and-hope: confirm the overlay actually
    # produced the architecture this script's own report text describes.
    mismatches = []
    for field, expected in (
        ("hidden_size", HIDDEN_SIZE), ("num_layers", NUM_LAYERS),
        ("num_heads", NUM_HEADS), ("num_kv_heads", NUM_KV_HEADS),
        ("head_dim", HEAD_DIM), ("moe_intermediate", MOE_INTERMEDIATE),
        ("liquid_warmup_steps", LIQUID_WARMUP_STEPS), ("max_steps", TOTAL_STEPS),
    ):
        actual = getattr(cfg, field, None)
        if actual != expected:
            mismatches.append(f"{field}: expected {expected!r}, got {actual!r}")
    if getattr(cfg, "liquid_layers_idx", None) != list(LIQUID_LAYER_IDS):
        mismatches.append(
            f"liquid_layers_idx: expected {LIQUID_LAYER_IDS!r}, got {getattr(cfg, 'liquid_layers_idx', None)!r}"
        )
    if mismatches:
        fail(
            f"MERTFORMER_MODEL_CONFIG={PILOT_MODEL_CONFIG_OVERLAY!r} overlay did not "
            "produce the expected pilot architecture (wrong overlay filename, overlay "
            "not found, or the yaml changed without updating this script's mirrored "
            "constants):\n  - " + "\n  - ".join(mismatches)
        )
    log(f"Overlay verified: cfg matches {PILOT_MODEL_CONFIG_OVERLAY} on every checked field.")

    # --- run-harness concerns NOT specified by the pilot yaml ------------
    ...
    run_config = {
        ...
        "generated_utc": utc_now(),
        "purpose": (
            "171M LR/divergence stabilization pilot (config/model/"
            f"{PILOT_MODEL_CONFIG_OVERLAY}) -- Go/No-Go gate before the real 45K run; "
            "NOT a capability run and NOT a 45K scaling proxy"
        ),
        "planned": {
    ...
    log(
        ...
        f"{NUM_KV_HEADS}kv | experts {MOE_EXPERTS}x{MOE_ACTIVE} at layers 2,5,8 | "
        f"mixer at layers {LIQUID_LAYER_IDS} | seq {SEQ_LEN} | "
...
def write_report(ctx: dict) -> None:
    ...
    add("")
    add(
        f"This is the 171M LR/divergence stabilization PILOT (config/model/"
        f"{PILOT_MODEL_CONFIG_OVERLAY}), a Go/No-Go gate before the real 45K run — "
        "NOT a capability run and NOT a 45K scaling proxy (see that yaml's own header). "
        "It answers one question: does the candidate LR regime (config/config.py: "
        "learning_rate, warmup_ratio, router_lr_multiplier) keep grad_norm finite and "
        "the loss descending on the FULL architecture (BitNet ternary + sparse 8-expert/"
        "top-2 MoE + Liquid/CfC + GQA) at the smallest scale considered architecturally "
        "representative. The verdict below is a mechanical read of the yaml's own "
        "documented gate: clip-hit rate < 5%, grad_norm never reaches inf, no persistent "
        "Liquid freeze, no MoE collapse, held-out loss descending, clean kill+resume. A "
        f"model of roughly {_fmt(console.get('param_millions'), 1)}M parameters trained "
        f"on ~{PLANNED_TOKENS/1e6:.0f}M tokens will still produce mostly incoherent, "
        "partially-grammatical text — that is expected and is not part of this gate. "
        "Loss spikes, expert-routing imbalance, and skipped NaN steps are listed under "
        "Observations and are not failures unless they terminated the run or breached "
        "the gate criteria above."
    )
    add("")
    add("## 2. Run summary")
    add("")
    ...
    add(f"| Experts / mixer | {MOE_EXPERTS} experts, top-{MOE_ACTIVE}, at layers 2, 5 and 8; recurrent mixer at layers {LIQUID_LAYER_IDS} |")
    ...
    add("")
    add(
        "Place the repository at `./repo` beside `preflight_run_pilot171m.py` (or set "
        "the clone toggle at the top of the file), delete `pilot171m_run_output.zip` "
        "if present, and run `python preflight_run_pilot171m.py`. The dataset slice is "
...
def main() -> int:
    ...
    install_tee()
    log("=" * 72)
    log("171M STABILIZATION PILOT  (single run, zero arguments)")
    log(f"Goal: does the candidate LR regime hold on the full architecture at {HIDDEN_SIZE}h/{NUM_LAYERS}L?")
    log("Not the goal: model quality/capability. Expect incoherent text from this checkpoint.")
    log("=" * 72)
