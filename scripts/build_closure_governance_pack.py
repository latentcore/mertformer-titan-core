...
def build_change_control_sop() -> str:
    return dedent(
        ...
        ## CHANGELOG.md Maintenance (added 2026-07-25 -- closes a real drift gap)

        `CHANGELOG.md`/`CHANGELOG_TR.md` are hand-maintained, NOT auto-regenerated by the closure
        ladder (unlike most of `reports/`). They went stale for a full month (2026-06-28 to
        2026-07-25) despite a large volume of real closure passes landing in `BACKLOG.md`/
        `DECISIONS.md` in that window -- nothing enforced keeping them in sync, and no gate caught
        the drift because nothing checks CHANGELOG content.
        - Any Class B/C/D/E pass (see Required Ladder above) that lands a real, substantive
          `BACKLOG.md`/`DECISIONS.md` entry should also add or update the current `## Unreleased -
          <date>` section in both `CHANGELOG.md` and `CHANGELOG_TR.md` (EN+TR parity, matching the
          `Added`/`Fixed`/`Changed`/`Validation` structure already in use).
        - A short summary pointing at `BACKLOG.md`/`DECISIONS.md` for full detail is enough --
          CHANGELOG is not meant to duplicate their pass-by-pass narrative, only to index it.
        - This is not a hard gate (no script currently checks it); it is a documented expectation
          so the next operator/AI does not need to rediscover the gap the way this pass did.
        """
    ).strip()
