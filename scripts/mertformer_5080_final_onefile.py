...
# EMBED-TIME SNAPSHOT (DURUSTLUK NOTU):
# Asagidaki sha256/line_count/bytes degerleri bu onefile URETILDIGI AN yakalanmistir.
# Bunlar guncel layers/ dosyalarina karsi CANLI parite/esitlik iddiasi DEGILDIR ve
# repodan sapmis olabilir (or. satir-sayimi kurali bu snapshot ile diger snapshot
# arasinda farkli olabilir). Canli parite icin reponun kendi manifestlerini/araclarini
# kullanin; parity_crosscheck_local_repo yalnizca dosya varligi/sembol arar, bu
# sayisal degerleri DOGRULAMAZ.
# EMBED-TIME SNAPSHOT (single source EN): sha256/line_count/bytes captured at
# onefile build time; NOT a live parity claim against the current repo and may drift.
...
def ensure_writable_dir(path: Path, label: str) -> None:
    ...
    try:
        ...
        try:
            ...
            # best-effort temp temizlik; probe yoksa/silinemiyorsa yoksay
            pass
...
def reset_device_peak_memory(device: str) -> None:
    if device == "cuda" and torch.cuda.is_available():
        try:
            ...
            # best-effort; bu CUDA/torch surumunde desteklenmiyorsa yoksay
            pass
    ...
    if len(step_files) > keep:
        for p in step_files[: len(step_files) - keep]:
            try:
                ...
                # best-effort; silinemese de devam et ama gorunurluk icin uyar
                print(f"[warn] dosya silinemedi: {exc!r}", file=sys.stderr)
    return removed
...
# EMBED-TIME SNAPSHOT (DURUSTLUK NOTU):
# Asagidaki gomulu manifest sha256/line_count/bytes degerleri bu onefile URETILDIGI AN
# yakalanmistir. Bunlar guncel repoya karsi CANLI parite/esitlik iddiasi DEGILDIR ve
# sapmis olabilir; ayrica EMBEDDED_LAYER_PARITY ile bu manifest arasinda line_count
# farki gorulebilir cunku satir-sayimi kurali iki snapshot arasinda farkli olabilir.
# Canli parite icin reponun kendi manifestlerini/araclarini kullanin.
# EMBED-TIME SNAPSHOT (single source EN): values captured at onefile build time;
# NOT a live parity claim against the current repo and may drift between snapshots.
