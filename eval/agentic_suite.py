...
def evaluate_static_stub() -> Dict[str, object]:
    """DEMO / NON-CLAIM stub: the responder below returns canned correct
    answers, so gate_pass here is guaranteed and does NOT reflect any real
    model behaviour. The output is marked stub_no_model and must not be used
    as a release/capability claim; use evaluate_with_callable() with a real
    responder for actual gating.
    """
    ...
    # Honest marker: this run used a hardcoded stub responder, not a real model.
    # Downstream consumers must treat gate_pass as a demo, not a capability claim.
    report["claim_status"] = "stub_no_model"
    report["is_claim"] = False
    report["note"] = (
        "Generated by evaluate_static_stub() with canned responses; "
        "gate_pass is a demo and is NOT a real-model release claim."
    )
    return report
