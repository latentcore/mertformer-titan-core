...
> ⚙️ **OTOMATİK ÜRETİLİR — ELLE DÜZENLEMEYİN.** Bu dosyanın tamamı `python3 scripts/extract_dataset_refs.py --fetch-metadata` komutuyla yeniden üretilir; bir sonraki çalıştırmada elle yapılan her değişiklik sessizce ezilir. Veriyi düzeltmek için kod tabanındaki referansı veya `datasets/LICENSES*.md`'yi düzeltip komutu tekrar çalıştırın.
